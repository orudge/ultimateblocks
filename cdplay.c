...
void CD_Player(void)
{
 ...
 if (cd_track == -1) textprintf_centre(temp2, fonts[0].dat, 100, 50,  0, "Play");
 else textprintf_centre(temp2, fonts[0].dat, 100, 50,  0, "Stop");
 ...
 while(!cd_exit)
 {
  ...
  if (key[KEY_ENTER])
  {
   ...
   if (item == 0)
   {
    ...
    {
     if (cd_play_from(1) != -1) cd_track = 1;
    }
   }
   ...
   if (item == 1)
   {
    ...
    if (cd_track > last) cd_track = first;

    if (cd_play_from(cd_track) == -1) cd_track = -1;
  ...
  else play_sample(sfx[SFX_CLICK].dat, 255, 128, 1000, 0);
 ...
 while (!cd_exit)
 {
  ...
  if (key[KEY_ENTER])
  {
   if (item != 2)
   {
    ...
    if (mod_track < 0) mod_track = mod_last;
    if (mod_track > mod_last) mod_track = 0;
